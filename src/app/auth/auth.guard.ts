...
export class AuthGuard implements CanActivate {
    ...
    public token$: Observable<string | undefined> = this.store.select(
        ({ authState }: AppStore) => authState.token,
    ).pipe(
        take(1)
    );

    constructor(private router: Router, private authService: AuthService, private store: Store<AppStore>) {}
    ...
    public canActivate(
        ...
        const observables: (Observable<boolean> | Observable<string | undefined>)[] = [this.authService.isAuthenticated(), this.token$];
        ...
        return forkJoin(observables).pipe(
            tap(async (results: (boolean | string | undefined)[]) => {
                ...
                if (!authenticated) {
                    ...
                    if (validToken) {
                        this.store.dispatch(authSuccess({ status: 200, resource: { token: validToken }}));
                    }
                }
            }),
